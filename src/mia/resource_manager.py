...
class ModelResource(ManagedResource[Any]):
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(f"Model resource {self.resource_id} ({self.model_type}) initialized")
        except Exception as e:
...
class DatabaseResource(ManagedResource[Any]):
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to initialize database resource {self.resource_id}: {e}")
            raise
...
class ResourceManager:
    ...
    def register_resource(self, resource: ManagedResource) -> str:
        ...
        with self._lock:
            if resource.resource_id in self.resources:
                raise ConfigurationError(f"Resource {resource.resource_id} already registered", 
            ...
            logger.info(f"Resource {resource.resource_id} ({resource.resource_type}) registered")
    ...
    def use_resource(self, resource_id: str):
        ...
        if not resource:
            raise ConfigurationError(f"Resource {resource_id} not found", "RESOURCE_NOT_FOUND")
    ...
    def _handle_resource_cleanup(self, resource_id: str) -> None:
        ...
        with self._lock:
            if resource_id in self.resources:
                logger.warning(f"Resource {resource_id} was garbage collected without proper cleanup")
                try:
                    del self.resources[resource_id]
                except KeyError:
                    pass
    ...
    def _check_memory_usage(self) -> None:
        ...
        if total_memory > self.max_memory_bytes:
            logger.warning(f"Memory usage ({total_memory / 1024 / 1024:.1f}MB) exceeds limit")
            self._free_memory()
        ...
        with self._lock:
            ...
            for resource in self.resources.values():
                try:
                    ...
                    logger.error(f"Error getting memory usage for {resource.resource_id}: {e}")
            return total
        ...
        with self._lock:
            ...
            for resource in self.resources.values():
                try:
                    ...
                    logger.error(f"Error getting info for resource {resource.resource_id}: {e}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'memory_usage_percent': (self.get_total_memory_usage() / self.max_memory_bytes) * 100,
