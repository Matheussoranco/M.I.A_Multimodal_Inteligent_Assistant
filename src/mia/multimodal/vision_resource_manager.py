...
class VisionResourceManager(ResourceManager):
    ...
    def acquire_vision_resource(
        self, name: str, vision_component: Any, config_manager=None
    ) -> VisionResource:
        ...
        while self._running:
            try:
                ...
                if total_memory > self.max_memory_bytes:
                    logger.warning(
                        f"Vision memory usage ({total_memory / 1024 / 1024:.1f}MB) exceeds limit"
                    )
                    self.cleanup_idle_resources()
