...
class AudioResourceManager(ResourceManager):
    ...
    def acquire_audio_resource(
        self, name: str, audio_component: Any, config_manager=None
    ) -> AudioResource:
    ...
    def cleanup_idle_resources(self):
        ...
        with self._lock:
            ...
            for name, resource in self.audio_resources.items():
                if (
                    not resource.is_recording
                    and not resource.is_playing
                    and current_time - resource.last_activity > idle_timeout
                ):
    ...
    def _audio_cleanup_thread(self):
        ...
        while self._running:
            try:
                ...
                # Check memory usage
                total_memory = sum(
                    resource.get_memory_usage()
                    for resource in self.audio_resources.values()
                )
                if total_memory > self.max_memory_bytes:
                    logger.warning(
                        f"Audio memory usage ({total_memory / 1024 / 1024:.1f}MB) exceeds limit"
                    )
                    self.cleanup_idle_resources()
