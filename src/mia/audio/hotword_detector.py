...
class HotwordDetector:
    ...
    def __init__(
        ...
        if not _HAS_RAPIDFUZZ and SequenceMatcher is None:
            logger.warning(
                "Fuzzy match libraries unavailable; hotword detection will rely on substring matches."
            )
    ...
    def _score_text(self, transcript: str) -> float:
        ...
        if (
            _HAS_RAPIDFUZZ and fuzz is not None
        ):  # pragma: no cover - depends on extra package
            ...
            score = (
                ...
                if cleaned.split()
                else SequenceMatcher(None, self.hotword, cleaned).ratio()
            )
        else:
    ...
    def detect(
        self, transcript: str, audio: Optional[np.ndarray] = None
    ) -> Optional[HotwordDetection]:
        ...
        if score >= self.sensitivity and (
            energy is None or energy >= self.energy_floor
        ):
            return HotwordDetection(
                confidence=score, transcript=transcript, energy=energy
            )
        return None
