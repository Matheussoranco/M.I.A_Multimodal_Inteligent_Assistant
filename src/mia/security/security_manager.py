...
class SecurityManager:
        ...
        try:
            ...
            if context:
                if not self._validate_context(action, context):
                    raise SecurityError(
                        f"Context validation failed for action: {action}",
            ...
            raise SecurityError(
                f"Permission check failed: {str(e)}", "PERMISSION_CHECK_ERROR"
            )
    ...
    def set_policy(self, action: str, allowed: bool):
        ...
        if not isinstance(allowed, bool):
            raise ValidationError(
                "Permission value must be boolean", "INVALID_PERMISSION"
            )
        ...
        try:
            ...
            raise ConfigurationError(
                f"Failed to set policy: {str(e)}", "POLICY_SET_FAILED"
            )
