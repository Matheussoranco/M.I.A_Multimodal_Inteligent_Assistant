    ...
    try:
        ...
        try:
            if LLMManager:
                ...
                logger.warning(
                    "LLM Manager not available - some features will be disabled"
                )
                llm = None
        except Exception as e:
        ...
        try:
            ...
            logger.warning(
                f"Some additional components failed to initialize: {e}"
            )
        ...
        while True:
            ...
            try:
                # Process image input
                if (
                    hasattr(args, "image_input")
                    and args.image_input
                    and vision_processor
                ):
                    try:
                        inputs["image"] = vision_processor.process_image(
                            args.image_input
                        )
                ...
                if (
                    ...
                    try:
                        ...
                        print(
                            red("❌ Audio processing failed. Switching to text mode.")
                        )
                        args.mode = "text"
                        continue
                else:
                    ...
                    if cmd == "quit":
                        ...
                        print(
                            f"  {bold('Model:')} {yellow(getattr(args, 'model_id', ''))}"
                        )
                        print(
                        ...
                        if cache_manager and hasattr(cache_manager, "get_stats"):
                            ...
                            hit_rate = cache_stats.get("memory_cache", {}).get(
                                "hit_rate", 0
                            )
                            print(
                        ...
                        args.mode = "audio"
                        print(
                            yellow("🎤 Switched to audio input mode. Say something...")
                        )
                        continue
                ...
                if inputs:
                    try:
                        ...
                        if input_text and action_executor:
                            agent_executed, agent_result = (
                                detect_and_execute_agent_commands(
                                    input_text, action_executor
                                )
                            )
                            if agent_executed:
                                print(agent_result)
        ...
        try:
            ...
            if (
                "performance_monitor" in locals()
                and performance_monitor
                and hasattr(performance_monitor, "stop_monitoring")
            ):
                performance_monitor.stop_monitoring()
            if (
                "performance_monitor" in locals()
                and performance_monitor
                and hasattr(performance_monitor, "cleanup")
            ):
                performance_monitor.cleanup()
            if (
                "cache_manager" in locals()
                and cache_manager
                and hasattr(cache_manager, "clear_all")
            ):
                cache_manager.clear_all()
            if (
                "resource_manager" in locals()
                and resource_manager
                and hasattr(resource_manager, "stop")
            ):
