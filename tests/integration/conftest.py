...
def cache_manager(test_data_dir):
    ...
    with patch("mia.cache_manager.PersistentCache") as MockPersistentCache:
        ...
        manager.persistent_cache = MockPersistentCache(
            cache_dir=test_data_dir, max_size_mb=10
        )
        return manager
...
def create_test_image(directory, filename="test.jpg"):
    ...
    with open(filepath, "wb") as f:
        # Minimal JPEG header
        f.write(
            b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x01\x00H\x00H\x00\x00\xff\xc0\x00\x11\x08\x00\x10\x00\x10\x01\x01\x11\x00\x02\x11\x01\x03\x11\x01\xff\xc4"
        )
    return filepath
