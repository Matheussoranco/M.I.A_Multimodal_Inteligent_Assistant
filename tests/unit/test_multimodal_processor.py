...
class TestMultimodalProcessor:
    ...
    @patch("speech_recognition.Recognizer")
    def test_process_audio_google_request_error_fallback_sphinx(
        self, mock_recognizer_class
    ):
    ...
    def test_analyze_emotion_high_energy_noisy(self):
        ...
        noisy_data = 0.3 * np.sin(
            2 * np.pi * 1000 * t
        )  # High frequency = high zero crossings
        mock_audio.get_raw_data.return_value = (
            (noisy_data * 32767).astype(np.int16).tobytes()
        )
    ...
    def test_analyze_emotion_high_energy_smooth(self):
        ...
        t = np.linspace(0, 0.1, 1000)
        smooth_data = 0.3 * np.sin(
            2 * np.pi * 50 * t
        )  # Low frequency = low zero crossings
        mock_audio.get_raw_data.return_value = (
            (smooth_data * 32767).astype(np.int16).tobytes()
        )
    ...
    def test_analyze_emotion_low_energy(self):
        ...
        mock_audio.get_raw_data.return_value = low_energy_data.astype(
            np.int16
        ).tobytes()
    ...
    def test_analyze_emotion_high_pitch(self):
        ...
        mock_audio.get_raw_data.return_value = (
            (high_pitch_data * 32767).astype(np.int16).tobytes()
        )
    ...
    def test_analyze_emotion_moderate_noise(self):
        ...
        mock_audio.get_raw_data.return_value = (
            (moderate_data * 32767).astype(np.int16).tobytes()
        )
    ...
    def test_analyze_emotion_neutral(self):
        ...
        mock_audio = Mock()
        t = np.linspace(0, 0.1, 1000)
        neutral_data = 0.005 * np.sin(
            2 * np.pi * 150 * t
        )  # Medium amplitude, medium frequency
        mock_audio.get_raw_data.return_value = (
            (neutral_data * 32767).astype(np.int16).tobytes()
        )
